...
export class HomeScreen extends PIXI.Container {
	constructor(startGame: () => void, newGame: () => void, settings: () => void) {
		...
		const newGameButton = new Button(50, "Nieuw Spel", undefined, undefined, newGame, 350);
		// Position 'New Game' Button
		newGameButton.position.set((1200 - newGameButton.width) / 2, (800 - newGameButton.height) / 2);
		// Add 'New Game' Button to the Homescreen
		this.addChild(newGameButton);

		// Create Settings Button
		const settingsButton = new Button(50, "Instellingen", undefined, undefined, settings, 350);
		// Position Settings Button
		settingsButton.position.set((1200 - settingsButton.width) / 2, (1000 - settingsButton.height) / 2);
